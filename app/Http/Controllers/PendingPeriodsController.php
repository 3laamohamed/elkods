...
class PendingPeriodsController extends Controller{
    ...
    public function closePeriod(Request $request){
        ...
        $this->incBorrow($request->customer,$request->valBorrow);
        $addNewPeriod = customerPeriods::create([
            'customer_id'=>$request->customer,
            'period_id'=>$getNewPeriod,
            'money'=>$request->priceValue + $request->valBorrow,
            'borrow'=>$request->valBorrow,
        ...
        if($addNewPeriod){
            ...
            $saveExchange = customerMoney::create([
                ...
                'time'=>$this->getTime(),
                'customer_id'=>$request->customer,
                'money'=>$request->priceValue,
                'note'=>'اذن قفل المدة',
                'user'=>$this->getUser()
            ]);
            if($saveExchange){
                $this->decreaseMoney($request->customer,$request->priceValue);
                customerClosePeriod::create([
                    'customer_id'=>$request->customer,
                    'date'=>$this->getDate()
                ]);
                return ['status'=>true];
